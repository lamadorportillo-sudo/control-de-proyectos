/* ===== PESTANA DISPONIBILIDAD PRESUPUESTARIA V2 ===== */
(()=>{
'use strict';
if(window.__CP_BUDGET_PORTFOLIO_TAB_V2__)return;
window.__CP_BUDGET_PORTFOLIO_TAB_V2__=true;
...
function ensureStyles(){
  if(document.getElementById('budget-portfolio-tab-v2-style'))return;
  const s=document.createElement('style');
  s.id='budget-portfolio-tab-v2-style';
  s.textContent=`
...
function removeLegacyPanel(){
  document.getElementById('budgetPortfolioPanel')?.remove();
}
...
function renderTabs(){
  ...
  if(!nav){
    ...
    nav.className='cp-main-tabs';
    nav.dataset.cpMainTabs='1';
    top.insertAdjacentElement('afterend',nav);
  }
  const budget=view.screen==='budgetPortfolio',n=projects().length;
  const sig=`${budget?'b':'p'}:${n}`;
  if(nav.dataset.sig!==sig){
    ...
    nav.querySelectorAll('[data-screen]').forEach(btn=>btn.addEventListener('click',()=>{
      view.screen=btn.dataset.screen;
      view.projectId=null;
...
function renderRows(){
  const body=document.getElementById('cpBudgetRows'),pager=document.getElementById('cpBudgetPager');
  if(!body||!pager)return;
  const all=filtered(),pages=Math.max(1,Math.ceil(all.length/state.perPage));
  state.page=Math.max(1,Math.min(state.page,pages));
  const start=(state.page-1)*state.perPage,show=all.slice(start,start+state.perPage);
  body.innerHTML=show.length?show.map(p=>{
    const v=snapshot(p),cls=v.disponible<-0.005?'neg':Math.abs(v.disponible)<=0.005?'zero':'',bar=Math.max(0,Math.min(100,v.pct));
    return `<tr><td class="code" data-label="Código"><button class="budget-row-btn" data-cp-budget-open="${h(p.id)}">${h(p.code||'—')}</button></td><td class="project" data-label="Proyecto"><div class="cp-budget-name">${h(p.name||'—')}</div><div class="cp-budget-sub">${h(p.budgetControl?.source||'Control presupuestario')}</div></td><td class="num" data-label="Vigente">${money(v.vigente)}</td><td class="num" data-label="Pagado">${money(v.paid)}</td><td class="num available ${cls}" data-label="Disponible">${money(v.disponible)}</td><td data-label="% pagado"><b>${v.pct.toFixed(2)}%</b><div class="cp-budget-progress"><i style="width:${bar}%"></i></div></td><td class="action" data-label="Acción"><button class="btn" data-cp-budget-open="${h(p.id)}">Ver saldo</button></td></tr>`;
  }).join(''):`<tr><td colspan="7" class="cp-budget-empty">No hay proyectos que coincidan con esta búsqueda.</td></tr>`;
  ...
  body.querySelectorAll('[data-cp-budget-open]').forEach(btn=>btn.addEventListener('click',()=>{
    view.projectId=btn.dataset.cpBudgetOpen;
    view.screen='project';
    view.tab='budget';
    if(typeof renderApp==='function')renderApp();
  }));
...
function renderBudgetScreen(){
  ...
  c.querySelectorAll('[data-f]').forEach(btn=>btn.addEventListener('click',()=>{
    state.filter=btn.dataset.f; state.page=1;
    c.querySelectorAll('[data-f]').forEach(x=>x.classList.toggle('active',x.dataset.f===state.filter));
    renderRows();
  }));
  renderRows();
}
