...
// Buscadores: filtrar sin redibujar conserva el foco.
...
if(html.includes(searchV3Old))html=html.replace(searchV3Old,searchV3New);
else if(!html.includes("grid.querySelectorAll('.project-v3')"))console.log('Aviso: no se encontró el manejador V3 esperado.');
...
.project-grid-v3{grid-template-columns:1fr!important;gap:8px!important}.view-switch{display:none!important}.project-v3{display:grid!important;grid-template-columns:minmax(0,1fr) auto!important;align-items:stretch!important;border-radius:14px!important;transform:none!important}.project-v3-main{display:grid!important;grid-template-columns:minmax(0,1.8fr) minmax(260px,.8fr)!important;gap:14px!important;align-items:center!important;padding:12px 14px!important}.project-v3 h3{font-size:13px!important;line-height:1.25!important;margin:5px 0 4px!important;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.project-v3-code{font-size:9px!important}.project-v3 .status{font-size:8px!important;padding:4px 7px!important}.project-v3-sub{font-size:8px!important;gap:6px!important}.project-v3-contractor,.project-v3-progress,.project-v3-health,.v3-words{display:none!important}.project-v3-money{grid-template-columns:1.2fr .7fr .7fr!important;gap:6px!important;margin:0!important}.v3-metric{padding:7px 8px!important}.v3-metric small{font-size:7px!important}.v3-metric b,.v3-metric.primary b{font-size:11px!important;margin-top:2px!important}.project-v3-actions{min-width:116px!important;display:flex!important;flex-direction:column!important;justify-content:center!important;gap:5px!important;padding:9px!important;border-top:0!important;border-left:1px solid rgba(148,163,184,.08)!important}.project-v3-actions .btn{min-height:30px!important;padding:6px 8px!important;font-size:8px!important;width:100%!important;margin:0!important}.project-v3-actions .btn.primary{order:-1!important}.project-grid-v3 .empty{grid-column:1/-1}@media(max-width:760px){.project-v3{display:block!important}.project-v3-main{display:block!important;padding:12px!important}.project-v3-money{margin-top:9px!important}.project-v3-actions{min-width:0!important;flex-direction:row!important;flex-wrap:wrap!important;border-left:0!important;border-top:1px solid rgba(148,163,184,.08)!important}.project-v3-actions .btn{width:auto!important;flex:1!important}.project-v3-actions .btn.primary{flex-basis:100%!important}}
</style>`;
if(!html.includes('projects-list-fix-v1'))html=html.replace('</head>',listCss+'\n</head>');

// Recursos de acceso: su versión viene del manifiesto.
...
if(html.includes(progressOld))html=html.replace(progressOld,progressOld);
...
if(start>=0){const endMarker='</script>\n</body></html>`;',end=html.indexOf(endMarker,start);if(end<0)throw new Error('Se detectó el escáner incrustado, pero no se encontró su cierre.');html=html.slice(0,start)+'<body>${body}</body></html>`;'+html.slice(end+endMarker.length)}
...
if(!html.includes('LECTOR DOCUMENTAL PROFUNDO V2')){const bodyEnd=html.toLowerCase().lastIndexOf('</body>');html=html.slice(0,bodyEnd)+`<script>\n${deepScanner}\n</script>\n`+html.slice(bodyEnd)}
...
if(!html.includes('APRENDIZAJE ADAPTATIVO V1')){const bodyEnd=html.toLowerCase().lastIndexOf('</body>');html=html.slice(0,bodyEnd)+`<script>\n${adaptiveLearning}\n</script>\n`+html.slice(bodyEnd)}

// Endurecimiento antes del primer render.
...
if(!html.includes('budget-portfolio-tab-v1.js')){const bodyEnd=html.toLowerCase().lastIndexOf('</body>');html=html.slice(0,bodyEnd)+'<script src="budget-portfolio-tab-v1.js"></script>\n'+html.slice(bodyEnd)}
...
const lateModules=[
  ['workspace-access-v1.js','20260820-master4'],['private-access-v1.js','20260823-private5'],['password-recovery-v1.js','20260822-password1'],['admin-users-v1.js','20260823-admin-users4'],['security-runtime-v1.js','20260823-security1'],['security-center-v1.js','20260823-security-center1'],['mfa-security-v1.js','20260824-mfa4'],['alerts-compact-v1.js','20260820-master4'],['dashboard-executive-v1.js','20260820-master4'],['engineering-ux-v1.js','20260820-master4'],['procurement-thresholds-v1.js','20260820-gacetas4'],['contracts-center-v1.js','20260820-contracts2'],['corporate-ui-v1.js','20260820-corporate3'],['corporate-polish-v1.js','20260820-polish6'],['procurement-award-fix-v1.js','20260820-award3'],['visit-photos-v1.js','20260820-visits1'],['award-notices-v1.js','20260820-notes1'],['visit-photo-persistence-v2.js','20260820-photopersist2'],['visit-print-v3.js','20260820-visitprint3'],['procurement-invitations-v2.js','20260820-invitations2'],['procurement-offers-invitees-v1.js','20260822-offersinvitees2'],['summary-budget-law-v1.js','20260820-summarybudgetlaw1'],['project-search-clean-v1.js','20260820-searchclean1'],['storage-quota-fix-v1.js','20260820-storagequota2'],['project-tabs-complete-v1.js','20260828-tabscomplete32'],['feature-lazy-loader-v1.js','20260824-lazy3'],['procurement-process-save-v4.js','20260820-procsave4'],['contract-integrity-fix-v1.js','20260822-integrity1'],['integrity-hardening-v2.js','20260822-integrity2'],['cross-module-sync-v1.js','20260822-relations1'],['programacion-control-v1.js','20260823-programacion4'],['web-knowledge-v2.js','20260822-short1'],['engineering-manual-reference-v1.js','20260823-manual1'],['adaptive-chat-learning-v1.js','20260822-global1'],['halu-page-controller-v1.js','20260824-control1'],['engineer-chatbot-v3.js','20260824-ai5'],['halu-avatar-motion-v1.js','20260822-place13'],['transparency-portal-v1.js','20260822-transparency1'],['photo-gallery-polish-v2.js','20260822-photopolish2'],['ui-theme-unifier-v1.js','20260822-theme1'],['engineering-visibility-fix-v1.js','20260822-visibility1'],['ui-operational-polish-v1.js','20260822-operational1'],['home-executive-fix-v2.js','20260822-home2'],['ui-visibility-audit-v1.js','20260822-uiaudit1'],['portfolio-redesign-v1.js','20260821-portfolio3'],['project-portfolio-detail-v1.js','20260821-projectdetail2'],['portfolio-gallery-v1.js','20260828-gallery3'],['portfolio-screen-fix-v1.js','20260821-screenfix1'],['project-photo-story-v1.js','20260821-photostory1'],
];
const activeLateModules=lateModules.filter(([module])=>!retiredModules.includes(module)).map(([module,version])=>[module,preAuthVersions.get(module)||version]);
...
for(const [module,version] of preAuthModules)if(!html.includes(`${module}?v=${version}`))throw new Error(`La versión canónica previa no quedó aplicada: ${module}?v=${version}`);

const scripts=[...html.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/gi)].map(m=>m[1]);
if(!scripts.length)throw new Error('No se encontraron scripts en el HTML final.');
for(let i=0;i<scripts.length;i++){try{new vm.Script(scripts[i],{filename:`inline-${i+1}.js`})}catch(err){throw new Error(`JavaScript inválido en bloque ${i+1}: ${err.message}`)}}
