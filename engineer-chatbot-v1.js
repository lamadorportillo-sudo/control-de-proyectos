...
function mount(){
  ...
  const ask=text=>{const q=String(text||'').trim();if(!q)return;add('user',q);setTimeout(()=>Promise.resolve(answer(q)).then(reply=>add('bot',reply,q)).catch(()=>add('bot','No pude completar la consulta. Intenta nuevamente.',q)),140)};
