...
function socialReply(q,spoken){
  ...
  if(/^(gracias|muchas gracias|perfecto|entendido)$/.test(spoken)){conversation.lastType='social';return`A la orden${name}. Aquí seguimos pendientes. ¿Cómo lo están manejando ustedes en el proyecto?`}
  ...
  if(/\b(tranquilo|tranquila|relajado|relajada|todo bien|muy bien|aqui bien|ahi vamos)\b/.test(spoken)&&(/\by tu\b/.test(spoken)||conversation.lastType==='social')){conversation.lastType='social';return`Pues qué bueno${name}. Yo aquí, pendiente de la obra y el papeleo que nunca se acaba. ¿Qué frente o asunto tienes entre manos hoy?`}
...
function answer(text){
  ...
  if(/\b(eres una ia|eres ia|inteligencia artificial|eres un bot|eres humano|persona real)\b/.test(spoken))return'Soy Halu, el asistente digital de ingeniería de este sistema. Tengo una forma de hablar de campo y memoria del espacio de trabajo, pero no voy a venderte humo diciendo que soy una persona. Lo útil es que puedo seguir el expediente, la norma y las decisiones que ustedes autoricen guardar. ¿Qué necesitas revisar?';
  const remember=q.match(/^recuerda que\s+(.+)/i);if(remember){const result=window.__ccChatLearning?.rememberFact?.(remember[1]);return result?.saved?`Anotado. Voy a recordar que ${result.text}. Eso queda compartido en este espacio de trabajo para retomarlo después. ¿Hay alguna decisión o fecha que también deba relacionar?`:'Eso no lo voy a guardar. Puede estar vacío o contener una contraseña, token o dato sensible. ¿Quieres redactarlo sin información privada?'}
  ...
  if(/qu[eé] puedes hacer|ayuda|opciones/i.test(q))return'Mira, puedo revisar el Decreto 62-2026, la Ley de Contratación y su Reglamento; también ubico módulos, abro pestañas y te ayudo a seguir el proceso. Si es asunto legal, te doy artículo y página, sin adornar lo que la norma no dice. ¿Qué estás revisando?';
  ...
  if(/relacion|sincron|actualiza/i.test(q))return'Mira, los módulos trabajan con el mismo expediente. Si cambias el monto mediante una modificación, eso debe reflejarse en resumen, pagos e informes. La teoría es bonita, pero si el dato se captura dos veces termina descuadrado; por eso aquí se reutiliza el mismo registro. ¿Qué relación quieres revisar?';
  ...
  const remembered=window.__ccChatLearning?.recall?.(q,2)||[];if(remembered.length)return`Mira, esto conecta con algo que el equipo dejó anotado: ${remembered.map(item=>item.text).join(' También quedó registrado que ')}. ¿Cómo siguió ese asunto en campo?`;
