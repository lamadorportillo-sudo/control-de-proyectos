...
function mount(){
  ...
  let busy=false;
  const ask=text=>{const q=String(text||'').trim();if(!q||busy)return;busy=true;sendButton.disabled=true;add('user',q);const typing=document.createElement('div');typing.className='cc-eng-msg bot';typing.textContent='Halu está pensando…';body.appendChild(typing);body.scrollTop=body.scrollHeight;setTimeout(()=>Promise.resolve(answerWithAI(q)).then(reply=>{typing.remove();const clean=String(reply||'').trim()||'No encontré una respuesta clara. Cuéntame un poco más.';const last=conversation.history[conversation.history.length-1];if(last?.role!=='assistant'||last.text!==clean)rememberTurn('assistant',clean);add('bot',clean,q)}).catch(()=>{typing.remove();const fallback='No pude completar la consulta. Intenta nuevamente.';rememberTurn('assistant',fallback);add('bot',fallback,q)}).finally(()=>{busy=false;sendButton.disabled=false;input.focus()}),140)};
