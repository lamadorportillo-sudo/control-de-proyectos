...
const CHAT_HISTORY_KEY='cc_zordon_chat_history_v1';
function loadConversationHistory(){try{const saved=JSON.parse(localStorage.getItem(CHAT_HISTORY_KEY)||'[]');return Array.isArray(saved)?saved.filter(turn=>turn&&['user','assistant'].includes(turn.role)&&typeof turn.text==='string').slice(-30):[]}catch{return[]}}
function saveConversationHistory(history){try{localStorage.setItem(CHAT_HISTORY_KEY,JSON.stringify(history.slice(-30)))}catch{}}
const conversation={lastTopic:'',lastType:'',turns:0,userName:'',history:loadConversationHistory()};
...
function rememberTurn(role,text){conversation.history.push({role,text:String(text||'').slice(0,900)});conversation.history=conversation.history.slice(-30);saveConversationHistory(conversation.history)}
function resetConversation(){
  conversation.lastTopic='';conversation.lastType='';conversation.turns=0;conversation.userName='';conversation.history=[];try{localStorage.removeItem(CHAT_HISTORY_KEY)}catch{}
...
function socialReply(q,spoken){
  ...
  if(/\b(quiero llorar|ganas de llorar|me siento fatal|me siento muy mal|no puedo mas|no puedo más|me duele mucho)\b/.test(spoken)){conversation.lastType='social';return`Uy${name}… aquí estoy. Si quieres, cuéntame qué te pegó así. No tienes que resolverlo todo de una vez.`}
...
function css(){
  ...
  .cc-eng-chat{position:fixed;right:164px;bottom:20px;z-index:121;width:min(390px,calc(100vw - 24px));height:min(610px,calc(100vh - 40px));display:none;grid-template-rows:auto 1fr auto;border:1px solid #29405a;border-radius:18px;overflow:hidden;background:#08111b;box-shadow:0 22px 70px rgba(0,0,0,.58);color:#eaf3fc}.cc-eng-chat.open{display:grid}.cc-eng-chat-head{display:flex;align-items:center;gap:10px;padding:10px 13px;border-bottom:1px solid #213247;background:linear-gradient(135deg,#10243b,#09131f)}.cc-eng-chat-head .cc-eng-avatar-frame{width:48px;height:70px;border:0}.cc-eng-chat-head>div{min-width:0;flex:1}.cc-eng-chat-head b,.cc-eng-chat-head small{display:block}.cc-eng-chat-head small{color:#83a0bb;font-size:9px;margin-top:2px}.cc-eng-chat-reset,.cc-eng-chat-close{border:0;background:transparent;color:#9fb5ca;font-size:18px;cursor:pointer;padding:5px;border-radius:8px}.cc-eng-chat-reset:hover,.cc-eng-chat-close:hover{background:#17324c;color:#fff}
  ...
  .cc-eng-chat-form{display:grid;grid-template-columns:auto auto minmax(0,1fr) auto;gap:7px;padding:10px;border-top:1px solid #213247;background:#09131f}.cc-eng-chat-form textarea{min-width:0;min-height:42px;max-height:112px;resize:none;border:1px solid #29405a;border-radius:11px;background:#060d15;color:#eef6ff;padding:10px 11px;font:11px/1.4 system-ui}.cc-eng-chat-form button{border:0;border-radius:11px;background:#2563eb;color:#fff;padding:0 14px;font-weight:850;cursor:pointer}.cc-eng-chat-form button:disabled{opacity:.55;cursor:wait}.cc-eng-chat-form .tool{padding:0 10px;background:#17324c;font-size:16px}.cc-eng-chat-form input[type="file"]{display:none}
...
function answer(text){
  ...
  if(/\b(eres una ia|eres ia|inteligencia artificial|eres un bot|eres humano|persona real)\b/.test(spoken))return'Soy ZORDON, el asistente digital del sistema. No soy una persona, pero puedo conversar contigo, recordar este hilo y ayudarte con el trabajo cuando lo necesites. ¿Qué revisamos?';
...
function mount(){
  ...
  const launch=document.createElement('button');launch.id='ccEngineerChatLaunch';launch.className='cc-eng-chat-launch';launch.title='Hablar con ZORDON';launch.setAttribute('aria-label','Hablar con ZORDON');launch.innerHTML='<span class="cc-eng-avatar-frame cc-halu-walker"><img class="cc-halu-layer cc-halu-torso" src="halu-engineer-cutout-v4.webp" alt="Halu, ingeniero civil de apoyo" decoding="async"><img class="cc-halu-layer cc-halu-leg cc-halu-leg-left" src="halu-engineer-cutout-v4.webp" alt="" decoding="async"><img class="cc-halu-layer cc-halu-leg cc-halu-leg-right" src="halu-engineer-cutout-v4.webp" alt="" decoding="async"><img class="cc-halu-layer cc-halu-seated-image" src="halu-engineer-seated-v1.webp" alt="Halu sentado" loading="lazy" decoding="async"></span><span class="dot"></span>';
  const box=document.createElement('section');box.id='ccEngineerChat';box.className='cc-eng-chat';box.setAttribute('aria-label','Chat con ZORDON');box.innerHTML=`<header class="cc-eng-chat-head"><span class="cc-eng-avatar-frame"><img src="halu-engineer-cutout-v4.webp" alt="Halu, ingeniero civil" loading="lazy" decoding="async"></span><div><b>ZORDON · Ingeniero Civil</b><small>Conversación continua · memoria y contexto del proyecto</small></div><button type="button" class="cc-eng-chat-reset" title="Iniciar una conversación nueva" aria-label="Nueva conversación">↻</button><button type="button" class="cc-eng-chat-close" aria-label="Cerrar">×</button></header><div class="cc-eng-chat-body" aria-live="polite"><div class="cc-eng-msg bot">Qué tal. Soy ZORDON. Podemos conversar de lo que quieras y también trabajar con los expedientes. Recordaré este hilo mientras sigamos hablando. ¿Cómo va todo?</div><div class="cc-eng-quick"><button data-q="¿Qué puedes controlar?">Controlar página</button><button data-q="Ponte aquí">Colocar avatar</button><button data-q="Voy a registrar una visita">Registrar visita</button><button data-q="Abre programa de costos">Programa de costos</button><button data-q="¿Dónde estoy?">¿Dónde estoy?</button><button data-q="Abre proyectos">Proyectos</button></div></div><form class="cc-eng-chat-form"><button type="button" class="tool" data-photo title="Agregar foto">📷</button><button type="button" class="tool" data-mic title="Dictar nota">🎙️</button><input type="file" accept="image/*" capture="environment" data-photo-input><textarea rows="1" autocomplete="off" maxlength="1200" placeholder="Cuéntame qué está pasando…" aria-label="Mensaje"></textarea><button type="submit">Enviar</button></form>`;
  ...
  let busy=false;
  const ask=text=>{const q=String(text||'').trim();if(!q||busy)return;busy=true;sendButton.disabled=true;add('user',q);const typing=document.createElement('div');typing.className='cc-eng-msg bot';typing.textContent='ZORDON está pensando…';body.appendChild(typing);body.scrollTop=body.scrollHeight;setTimeout(()=>Promise.resolve(answerWithAI(q)).then(reply=>{typing.remove();const clean=String(reply||'').trim()||'No encontré una respuesta clara. Cuéntame un poco más.';const last=conversation.history[conversation.history.length-1];if(last?.role!=='assistant'||last.text!==clean)rememberTurn('assistant',clean);add('bot',clean,q)}).catch(()=>{typing.remove();const fallback='No pude completar la consulta. Intenta nuevamente.';rememberTurn('assistant',fallback);add('bot',fallback,q)}).finally(()=>{busy=false;sendButton.disabled=false;input.focus()}),140)};
  ...
  box.querySelector('.cc-eng-chat-reset').onclick=()=>{if(fieldVisit&&!confirm('Hay una visita en curso. ¿Descartarla e iniciar una conversación nueva?'))return;resetConversation();add('bot','Listo, comenzamos un hilo nuevo. ¿Qué quieres revisar?')};
  ...
  input.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();box.querySelector('form').requestSubmit()}});
