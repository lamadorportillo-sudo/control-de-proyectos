...
function css(){
  ...
  .cc-eng-chat-launch{position:fixed;right:20px;bottom:20px;z-index:120;width:62px;height:62px;border:2px solid #38bdf8;border-radius:50%;padding:0;overflow:hidden;background:#07111d;box-shadow:0 14px 38px rgba(0,0,0,.45),0 0 0 5px rgba(56,189,248,.10);cursor:pointer}.cc-eng-avatar-frame{position:relative;display:block;flex:0 0 auto;overflow:hidden;border-radius:50%;background:#e8edf2}.cc-eng-chat-launch .cc-eng-avatar-frame{width:100%;height:100%}.cc-eng-avatar-frame img{position:absolute;width:100%;height:100%;max-width:none;left:0;top:0;object-fit:contain;object-position:center}.cc-eng-chat-launch .dot{position:absolute;right:2px;bottom:3px;width:13px;height:13px;border:2px solid #07111d;border-radius:50%;background:#22c55e}
  .cc-eng-chat{position:fixed;right:20px;bottom:94px;z-index:121;width:min(390px,calc(100vw - 24px));height:min(610px,calc(100vh - 120px));display:none;grid-template-rows:auto 1fr auto;border:1px solid #29405a;border-radius:18px;overflow:hidden;background:#08111b;box-shadow:0 22px 70px rgba(0,0,0,.58);color:#eaf3fc}.cc-eng-chat.open{display:grid}.cc-eng-chat-head{display:flex;align-items:center;gap:10px;padding:12px 13px;border-bottom:1px solid #213247;background:linear-gradient(135deg,#10243b,#09131f)}.cc-eng-chat-head .cc-eng-avatar-frame{width:43px;height:43px;border:1px solid #38bdf8}.cc-eng-chat-head>div{min-width:0;flex:1}.cc-eng-chat-head b,.cc-eng-chat-head small{display:block}.cc-eng-chat-head small{color:#83a0bb;font-size:9px;margin-top:2px}.cc-eng-chat-close{border:0;background:transparent;color:#9fb5ca;font-size:20px;cursor:pointer}
...
function mount(){
  ...
  const launch=document.createElement('button');launch.id='ccEngineerChatLaunch';launch.className='cc-eng-chat-launch';launch.title='Hablar con Halu';launch.setAttribute('aria-label','Hablar con Halu');launch.innerHTML='<span class="cc-eng-avatar-frame"><img src="halu-engineer-avatar-v2.png" alt="Halu, ingeniero civil de apoyo"></span><span class="dot"></span>';
  const box=document.createElement('section');box.id='ccEngineerChat';box.className='cc-eng-chat';box.setAttribute('aria-label','Chat con Halu');box.innerHTML=`<header class="cc-eng-chat-head"><span class="cc-eng-avatar-frame"><img src="halu-engineer-avatar-v2.png" alt="Halu, ingeniero civil"></span><div><b>Halu · Ingeniero Civil</b><small>Obra, contratos y control de proyectos</small></div><button class="cc-eng-chat-close" aria-label="Cerrar">×</button></header><div class="cc-eng-chat-body"><div class="cc-eng-msg bot">Qué tal. Soy Halu. Aquí hablamos directo: obra, contratos, estimaciones y los problemas que en el plano no aparecen. ¿Cómo va todo?</div><div class="cc-eng-quick"><button data-q="Voy a registrar una visita">Registrar visita</button><button data-q="¿Dónde estoy?">¿Dónde estoy?</button><button data-q="¿Qué regula la garantía de cumplimiento?">Consulta legal</button><button data-q="Abre proyectos">Proyectos</button></div></div><form class="cc-eng-chat-form"><button type="button" class="tool" data-photo title="Agregar foto">📷</button><button type="button" class="tool" data-mic title="Dictar nota">🎙️</button><input type="file" accept="image/*" capture="environment" data-photo-input><input type="text" autocomplete="off" maxlength="500" placeholder="Cuéntame qué está pasando…" aria-label="Mensaje"><button>Enviar</button></form>`;
