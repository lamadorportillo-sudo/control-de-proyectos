...
import { hasSupabaseConfig, ensureSupabaseSession, getV2AuthState } from './supabaseClient.ts';
...
export class LocalDataRepository implements IDataRepository {
  async getProjects(): Promise<Project[]> { return appStore.getProjects(); }
  async getProjectById(id: string): Promise<Project | undefined> { return appStore.getProjectById(id); }
  async saveProject(project: Project): Promise<void> { appStore.saveProject(project); }
  async getContracts(): Promise<Contract[]> { return appStore.getContracts(); }
  async getContractByProjectId(projectId: string): Promise<Contract | undefined> { return appStore.getContractByProjectId(projectId); }
  async saveContract(contract: Contract): Promise<void> { appStore.saveContract(contract); }
  ...
  async saveEstimate(estimate: Estimate): Promise<void> { appStore.saveEstimate(estimate); }
  ...
  async saveGuarantee(guarantee: Guarantee): Promise<void> { appStore.saveGuarantee(guarantee); }
  ...
  async saveDeficiency(deficiency: Deficiency): Promise<void> { appStore.saveDeficiency(deficiency); }
  async getDocuments(projectId?: string): Promise<DocumentEvidence[]> {
    const all = appStore.getAllDocuments();
    return projectId ? all.filter((d) => d.projectId === projectId) : all;
  }
  async saveDocument(doc: DocumentEvidence): Promise<void> { appStore.saveDocument(doc); }
  ...
  async saveFieldVisit(visit: FieldVisit): Promise<void> { appStore.saveVisit(visit); }
  async getAuditLogs(): Promise<AuditLog[]> { return appStore.getAuditLogs(); }
  async logAction(action: Omit<AuditLog, 'id' | 'timestamp'>): Promise<void> {
    appStore.logAudit(action.action, action.entityType, action.entityId, action.entityCode, action.details);
  }
}
...
export const telegramAdapter: ITelegramAdapter = {
  ...
  sendDataToBot(data: object): void {
    if (this.isAvailable()) (window as any).Telegram.WebApp.sendData(JSON.stringify(data));
  },
  closeMiniApp(): void {
    if (this.isAvailable()) (window as any).Telegram.WebApp.close();
  },
  expandMiniApp(): void {
    if (this.isAvailable()) (window as any).Telegram.WebApp.expand();
  },
};
    ...
    get(target, property, receiver) {
      ...
      return async (...args: unknown[]) => {
        ...
        if (!auth.authenticated) {
          throw new Error('Sesión requerida. Abre la V2 desde una sesión iniciada en Control Contractual.');
        }
