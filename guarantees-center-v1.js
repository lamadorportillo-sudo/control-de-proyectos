...
const contractFor=(list,projectId,preferredId='')=>{
  const all=A(list).filter(x=>String(x.projectId||'')===String(projectId||''));
...
function rows(){const d=DB()||{},ps=A(d.projects).filter(p=>!p.deletedAt),cs=A(d.contracts);return A(d.guarantees).filter(g=>!g.voidedAt&&!g.voided_at).map(g=>{const p=ps.find(x=>x.id===g.projectId)||{},c=contractFor(cs,g.projectId,g.contractId)||{},a=alertFor(g.end);return{...g,projectCode:p.code||'',projectName:p.name||'Proyecto',contractNumber:c.number||'',contractor:c.contractor||'No registrado',alert:a,amount:N(g.applied??g.calculated),extensions:A(g.extensions).length}}).filter(x=>x.projectName).sort((a,b)=>a.alert.days-b.alert.days)}
