...
function derivedAmount(c){
  if(!c)return 0;
  return R(N(c.originalAmount)+approvedChanges(c.id).reduce((sum,x)=>sum+N(x.amountDelta),0));
}
function storedAmount(c){
  if(!c)return 0;
  const candidates=[c.currentAmount,c.legacyCurrentAmount,c.legacy_current_amount];
  for(const value of candidates){if(value!==undefined&&value!==null&&String(value)!=='')return R(value)}
...
function scanProject(projectId){
  ...
  for(const c of contracts){
    ...
    if(Math.abs(stored-derived)>0.01){
      issues.push({
        level:'danger',kind:'contract_amount',projectId:p.id,contractId:c.id,
        title:'Monto contractual requiere revisión documental',
        detail:`El monto vigente almacenado (${money(stored)}) no coincide con monto original + modificaciones aprobadas (${money(derived)}). El sistema no lo corregirá automáticamente.`
      });
    }
    ...
    if(start&&end&&days>0&&span>0&&Math.abs(span-days)>1){
      issues.push({
        level:'warn',kind:'contract_dates',projectId:p.id,contractId:c.id,
        title:'Plazo y fechas requieren verificación',
...
function mountProjectNotice(){
  ...
  const box=document.createElement('section');box.dataset.ccIntegrityDiagnostics='1';box.className='panel';
