...
let syncing=false;
function activeRoute(){if(document.body.classList.contains('cc-contracts-center-active'))return'contratos';if(document.body.classList.contains('cc-payments-center-active'))return'pagos';if(document.body.classList.contains('cc-guarantees-center-active'))return'garantias';return''}
function sync(){
...
function closeCenters(except=''){if(except!=='contratos')window.__ccContractsCenter?.close?.();if(except!=='pagos')window.__ccPaymentsCenter?.close?.();if(except!=='garantias')window.__ccGuaranteesCenter?.close?.()}
