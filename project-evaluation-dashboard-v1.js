/* ===== DASHBOARD EVALUATIVO POR PROYECTO V1 ===== */
(()=>{
'use strict';
if(window.__CC_PROJECT_EVALUATION_DASHBOARD_V1__)return;
...
function fallbackFinancials(project,contract,estimates){
  ...
  return{
    grossC,netC,currentC,advancePaidC,advanceAmortizedC,
    totalPaidC:advancePaidC+paidEstimatesC,
    advancePendingC:Math.max(0,advancePaidC-advanceAmortizedC),
    saldoEstimarC:Math.max(0,currentC-grossC),
  };
}
...
function evaluateProject(projectId,cutoff=todayISO()){
  ...
  if(contract){
    procurementScore=100;
    procurementDetail='El proceso culminó con un contrato registrado.';
  ...
  if(advancePaidC>0||/pagado/i.test(contract?.advanceStatus||'')){
    const target=Math.max(1,N(contract?.recoveryTarget||80));
    const expected=clamp(financialProgress/target*100);
    const amortizationScore=clamp(100-Math.max(0,expected-N(advanceProgress))*2);
    advanceScore=clamp(amortizationScore-(advanceGuarantee?0:35));
  }

  const requiredGuarantees=[];
  if(contract){
    requiredGuarantees.push({label:'Cumplimiento',value:guarantees.find(item=>/cumplimiento/i.test(item.type||''))||null});
    if(advancePaidC>0)requiredGuarantees.push({label:'Anticipo',value:advanceGuarantee});
    if(isFinished||present(contract?.controls?.provisionalReceptionDate))requiredGuarantees.push({label:'Calidad',value:guarantees.find(item=>/calidad/i.test(item.type||''))||null});
  }
  const guaranteeProcessScore=requiredGuarantees.length?requiredGuarantees.reduce((sum,item)=>sum+guaranteeScore(item.value,cutoff),0)/requiredGuarantees.length:null;
  ...
  const processes=[
    ...
    processItem('advance','Anticipo y amortización',10,advanceScore,advancePaidC?`Pagado ${moneyCents(advancePaidC)} · amortizado ${percent(advanceProgress)} · pendiente ${moneyCents(advancePendingC)}.`:'No existe anticipo pagado registrado.','estimates'),
    processItem('guarantees','Garantías',10,guaranteeProcessScore,requiredGuarantees.length?requiredGuarantees.map(item=>`${item.label}: ${item.value?dateLabel(item.value.end):'no registrada'}`).join(' · '):'No corresponde evaluar garantías en esta etapa.','guarantees'),
  ...
  if(advancePaidC>0&&!advanceGuarantee)alert('danger','Falta garantía de anticipo','Existe anticipo pagado, pero no aparece su garantía.','guarantees');
  if(advancePaidC>0&&financialProgress>=80&&advancePendingC>1)alert('danger','Anticipo sin amortizar completamente',`Queda pendiente ${moneyCents(advancePendingC)} después del 80% de avance financiero.`,'estimates');
  requiredGuarantees.forEach(item=>{
    if(!item.value)return alert('danger',`Falta garantía de ${item.label.toLowerCase()}`,'Debe incorporarse al expediente contractual.','guarantees');
  ...
  if(changePct>25)alert('danger','Modificaciones sobre el límite de control',`Las variaciones aprobadas acumulan ${changePct.toFixed(1)}% del monto original.`,'changes');
  else if(changePct>10)alert('warning','Verificar adenda contractual',`Las variaciones aprobadas acumulan ${changePct.toFixed(1)}% del monto original.`,'changes');
  ...
  return{
    project,contract,estimates,visits,guarantees,changes,processes,alerts,
    overall,overallMeta,applicableCount:applicable.length,totalProcessCount:processes.length,
    physicalProgress,financialProgress,paidProgress,timeProgress,scheduleGap,
    currentC,grossC,totalPaidC,cutoff,latestVisit,pendingObservations,
  };
}
...
function css(){
  ...
  style.id='cc-project-evaluation-dashboard-style';
  style.textContent=`
    ...
    .ped-head{display:grid;grid-template-columns:1fr auto;gap:20px;align-items:center;padding:20px 22px;background:linear-gradient(118deg,#10243f,#185f9d);color:#fff}
    .ped-eyebrow{margin:0 0 4px;font-size:10px;font-weight:900;letter-spacing:.12em;color:#d9e9f7}.ped-head h3{margin:0;color:#fff;font-size:22px}.ped-head p:last-child{margin:5px 0 0;color:#d9e7f4;font-size:11px}
    .ped-health{--score:0deg;width:92px;height:92px;border-radius:50%;display:grid;place-items:center;background:conic-gradient(#43a66e var(--score),rgba(255,255,255,.18) 0);position:relative;box-shadow:inset 0 0 0 1px rgba(255,255,255,.14)}
    .ped-health:after{content:'';position:absolute;inset:8px;border-radius:50%;background:#fff}.ped-health>span{position:relative;z-index:1;text-align:center;color:#10243f}.ped-health b{display:block;font-size:24px;line-height:1}.ped-health small{display:block;margin-top:4px;font-size:8px;font-weight:900;text-transform:uppercase}
    .ped-kpis{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:1px;background:#dbe4ee;border-bottom:1px solid #d6e0ea}.ped-kpi{background:#fff;padding:14px 15px;min-width:0}.ped-kpi small{display:block;color:#6a7786;font-size:9px;font-weight:900;text-transform:uppercase;letter-spacing:.055em}.ped-kpi strong{display:block;margin-top:5px;color:#173b63;font-size:18px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ped-kpi span{display:block;margin-top:3px;color:#708091;font-size:9px}
    .ped-body{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(280px,.75fr);gap:14px;padding:15px}.ped-panel{background:#fff;border:1px solid #dbe3ec;border-radius:14px;padding:14px;min-width:0}.ped-panel-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:12px}.ped-panel-head h4{margin:0;color:#17324f;font-size:14px}.ped-panel-head p{margin:3px 0 0;color:#738191;font-size:9px}.ped-badge{border-radius:999px;padding:5px 8px;font-size:8px;font-weight:900;white-space:nowrap}.ped-badge.good,.ped-badge.ok{background:#e0f3e8;color:#236a49}.ped-badge.warn{background:#fff2cf;color:#9a5a00}.ped-badge.danger{background:#fde5e2;color:#a1281d}.ped-badge.na{background:#edf1f5;color:#667483}
    .ped-comparison{display:grid;gap:10px;margin-bottom:16px}.ped-compare-row{display:grid;grid-template-columns:110px minmax(0,1fr) 54px;align-items:center;gap:9px}.ped-compare-row span{font-size:10px;font-weight:800;color:#4d5f71}.ped-compare-row b{text-align:right;font-size:10px;color:#243b53}.ped-track{height:9px;border-radius:999px;background:#e7edf3;overflow:hidden}.ped-track i{display:block;height:100%;border-radius:999px;background:#185f9d}.ped-track i.time{background:#d8a629}.ped-track i.financial{background:#2b7a55}.ped-track i.paid{background:#7b61a8}
    .ped-process-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.ped-process{border:1px solid #e0e6ed;border-radius:11px;padding:10px;background:#fbfcfe;min-width:0}.ped-process-top{display:flex;justify-content:space-between;gap:8px;align-items:flex-start}.ped-process h5{margin:0;color:#213852;font-size:10px}.ped-process-score{font-size:10px;font-weight:900;color:#173b63;white-space:nowrap}.ped-process .ped-track{height:6px;margin:8px 0}.ped-process.good .ped-track i,.ped-process.ok .ped-track i{background:#2b7a55}.ped-process.warn .ped-track i{background:#d8a629}.ped-process.danger .ped-track i{background:#b42318}.ped-process.na .ped-track i{background:#a8b3bf}.ped-process p{margin:0;color:#68788a;font-size:8.5px;line-height:1.35;min-height:23px}.ped-process button{margin-top:7px;border:0;background:transparent;color:#185f9d;font-size:8.5px;font-weight:900;padding:0;cursor:pointer}
    .ped-alerts{display:grid;gap:8px}.ped-alert{display:grid;grid-template-columns:9px minmax(0,1fr) auto;gap:9px;align-items:center;width:100%;border:1px solid #e1e7ed;border-radius:11px;background:#fff;padding:10px;text-align:left;color:#243b53;cursor:pointer}.ped-alert>i{width:8px;height:8px;border-radius:50%;background:#d8a629}.ped-alert.danger>i{background:#b42318}.ped-alert.good>i{background:#2b7a55}.ped-alert b{display:block;font-size:9.5px}.ped-alert small{display:block;margin-top:2px;color:#6f7e8c;font-size:8.5px;line-height:1.3}.ped-alert>span:last-child{font-weight:900;color:#185f9d}.ped-empty{padding:14px;border-radius:11px;background:#e2f3e9;color:#236a49;font-size:10px;font-weight:800}.ped-foot{display:flex;justify-content:space-between;gap:12px;align-items:center;margin-top:12px;padding-top:10px;border-top:1px solid #e2e7ed;color:#748292;font-size:8.5px}
    @media(max-width:1050px){.ped-kpis{grid-template-columns:repeat(3,1fr)}.ped-body{grid-template-columns:1fr}.ped-process-grid{grid-template-columns:repeat(3,1fr)}}
    @media(max-width:760px){.ped-head{padding:17px}.ped-health{width:78px;height:78px}.ped-kpis{grid-template-columns:1fr 1fr}.ped-body{padding:10px}.ped-process-grid{grid-template-columns:1fr 1fr}.ped-kpi strong{font-size:15px}}
    @media(max-width:480px){.ped-head{grid-template-columns:1fr}.ped-health{width:72px;height:72px}.ped-kpis,.ped-process-grid{grid-template-columns:1fr}.ped-compare-row{grid-template-columns:90px minmax(0,1fr) 48px}.ped-foot{display:block}.ped-foot span{display:block;margin-top:4px}}
  `;
...
function comparisonRow(label,value,className=''){
  ...
  const shown=available?clamp(value):0;
...
function dashboardHTML(model){
  ...
  return`<section class="ped-shell" data-project-evaluation="${H(model.project.id)}" aria-label="Dashboard evaluativo del proyecto">
    <header class="ped-head"><div><p class="ped-eyebrow">CONTROL INTEGRAL AUTOMÁTICO</p><h3>Evaluación del proyecto</h3><p>Resultado calculado con el contrato, plazo, estimaciones, garantías, visitas, observaciones y modificaciones registradas.</p></div><div class="ped-health" style="--score:${clamp(model.overall)*3.6}deg"><span><b>${Math.round(model.overall)}</b><small>${H(model.overallMeta.label)}</small></span></div></header>
    <div class="ped-kpis">
      <article class="ped-kpi"><small>Avance físico</small><strong>${percent(model.physicalProgress)}</strong><span>${model.latestVisit?'Última visita registrada':'Cálculo desde estimaciones'}</span></article>
      <article class="ped-kpi"><small>Avance financiero</small><strong>${percent(model.financialProgress)}</strong><span>${model.estimates.length} estimación${model.estimates.length===1?'':'es'}</span></article>
      <article class="ped-kpi"><small>Tiempo consumido</small><strong>${model.timeProgress===null?'SIN DATOS':percent(model.timeProgress)}</strong><span>Corte ${dateLabel(model.cutoff)}</span></article>
      <article class="ped-kpi"><small>Total pagado</small><strong>${moneyCents(model.totalPaidC)}</strong><span>${percent(model.paidProgress)} del monto vigente</span></article>
      <article class="ped-kpi"><small>Alertas activas</small><strong>${model.alerts.length}</strong><span>${model.alerts.filter(item=>item.level==='danger').length} crítica${model.alerts.filter(item=>item.level==='danger').length===1?'':'s'}</span></article>
    </div>
    <div class="ped-body">
      <div class="ped-panel"><div class="ped-panel-head"><div><h4>Evaluación por procesos</h4><p>${model.applicableCount} de ${model.totalProcessCount} procesos evaluables con los datos actuales.</p></div><span class="ped-badge ${H(model.overallMeta.className)}">${Math.round(model.overall)}/100</span></div>
        <div class="ped-comparison">${comparisonRow('Tiempo',model.timeProgress,'time')}${comparisonRow('Avance físico',model.physicalProgress,'')}${comparisonRow('Avance financiero',model.financialProgress,'financial')}${comparisonRow('Pagado',model.paidProgress,'paid')}</div>
        <div class="ped-process-grid">${model.processes.map(item=>`<article class="ped-process ${H(item.className)}"><div class="ped-process-top"><h5>${H(item.label)}</h5><span class="ped-process-score">${item.score===null?'—':Math.round(item.score)+'/100'}</span></div><div class="ped-track" role="progressbar" aria-label="${H(item.label)}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${item.score===null?0:item.score.toFixed(1)}"><i style="width:${item.score===null?0:item.score}%"></i></div><p>${H(item.detail)}</p><button type="button" data-eval-jump="${H(item.tab)}">Abrir ${H(item.label.toLowerCase())} →</button></article>`).join('')}</div>
      </div>
      <aside class="ped-panel"><div class="ped-panel-head"><div><h4>Prioridades de seguimiento</h4><p>Las alertas abren directamente la sección que debe revisarse.</p></div><span class="ped-badge ${model.alerts.some(item=>item.level==='danger')?'danger':model.alerts.length?'warn':'good'}">${model.alerts.length||'OK'}</span></div><div class="ped-alerts">${topAlerts.length?topAlerts.map(item=>`<button type="button" class="ped-alert ${H(item.level)}" data-eval-jump="${H(item.tab)}"><i></i><span><b>${H(item.title)}</b><small>${H(item.detail)}</small></span><span>›</span></button>`).join(''):'<div class="ped-empty">✓ No se detectan asuntos críticos con la información registrada.</div>'}</div><div class="ped-foot"><b>${H(model.project.code||'Proyecto')}</b><span>Actualización automática · datos del expediente</span></div></aside>
    </div>
  </section>`;
}
...
function signature(model){
  return JSON.stringify([
    model.project.id,model.project.updatedAt,model.project.status,model.contract?.updatedAt,
    model.estimates.length,model.estimates.at(-1)?.updatedAt,model.visits.length,model.latestVisit?.updatedAt,
    model.guarantees.length,model.guarantees.map(item=>item.end).join('|'),model.changes.length,
    model.pendingObservations.length,Math.round(model.overall*100),model.alerts.length,
  ]);
}
function jump(tab){
  const button=document.querySelector(`[data-tab="${String(tab||'summary').replace(/"/g,'')}"]`);
  if(button){button.click();setTimeout(()=>document.querySelector('nav.tabs')?.scrollIntoView({behavior:'smooth',block:'start'}),60)}
}
function render(){
  let screen='',projectId='';
  try{screen=view?.screen||'';projectId=view?.projectId||''}catch{}
  const content=document.getElementById('content');
  if(!content||screen!=='project'||!projectId)return;
  const model=evaluateProject(projectId);
  if(!model)return;
  css();
  const nextSignature=signature(model);
  let section=content.querySelector('[data-project-evaluation]');
  if(section?.dataset.evalSignature===nextSignature)return;
  const wrapper=document.createElement('div');
  wrapper.innerHTML=dashboardHTML(model);
  const replacement=wrapper.firstElementChild;
  replacement.dataset.evalSignature=nextSignature;
  if(section)section.replaceWith(replacement);
  else{
    const actions=content.querySelector('.project-portfolio-actions');
    const kpis=content.querySelector('.grid-kpi');
    if(actions)actions.insertAdjacentElement('afterend',replacement);
    else if(kpis)content.insertBefore(replacement,kpis);
    else content.appendChild(replacement);
  }
...
function schedule(){
  if(queued)return;
  queued=true;
  queueMicrotask(()=>{queued=false;try{render()}catch(error){console.warn('No se pudo actualizar el dashboard evaluativo.',error)}});
}
...
window.__ccProjectEvaluationDashboard={evaluateProject,render,scoreMeta};
schedule();
})();
