...
function exact(file,from,to,label){let s=read(file);if(!s.includes(from))throw new Error(`${file}: no se encontró ${label}`);write(file,s.replace(from,to));}
function regex(file,re,to,label){let s=read(file);if(!re.test(s))throw new Error(`${file}: no se encontró ${label}`);write(file,s.replace(re,to));}
...
exact(docs,
  "`procedentes de la fuente ${X(ctl.financingSource||'Fondos Municipales')}`",
  "`procedentes de la fuente ${X(ctl.financingSource||'')}`",
  'financiamiento en contrato');
exact(docs,
  '`${X(wordsDays(c.executionDays||90))} `',
  '`${X(wordsDays(c.executionDays))} `',
  'plazo sin 90 días por defecto');
exact(docs,
  ...
  'financiamiento en orden de inicio');
exact(docs,
