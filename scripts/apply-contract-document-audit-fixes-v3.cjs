...
function exact(file,from,to,label){const s=read(file);if(!s.includes(from))throw new Error(`${file}: no se encontró ${label}`);write(file,s.replace(from,to));}
function regex(file,re,to,label){const s=read(file);if(!re.test(s))throw new Error(`${file}: no se encontró ${label}`);write(file,s.replace(re,to));}
...
'datos de nota y orden sin precarga implícita');
...
'mensaje de perfil por documento');
...
for(const from of [
  ...
  const to=from.replace(' required','');
  exact(docs,from,to,`campo opcional ${from.match(/id=\"([^\"]+)/)?.[1]||from}`);
}
...
'fecha de nota sin hoy implícito');
exact(docs,
...
'sexo explícito al abrir perfil');
...
'generación con requisitos específicos');
...
'monto original en contrato Word');
exact(docs,
...
'plazo sin un día implícito');

const manifest='authenticated-module-manifest-v1.cjs';
exact(manifest,
...
'orden seguro del plan autenticado');
...
"'contract-official-format-v1.js?v=20260831-phone3',\n'contract-document-safety-v1.js?v=20260904-docsafety3',\n'contract-payment-documents-v1.js?v=20260904-advance-docs4',\n'contract-preview-v1.js?v=20260831-preview1'",
'orden seguro en metadatos de pestañas');

for(const file of [docs,manifest,tabs])execFileSync(process.execPath,['--check',file],{stdio:'inherit'});
console.log('Requisitos específicos, monto original y orden de carga seguros aplicados.');
