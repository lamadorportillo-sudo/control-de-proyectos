/* ===== CENTRO DE SEGURIDAD Y ACCESOS V3 ===== */
(()=>{
'use strict';
...
async function emergencyLockdown(){if(typeof sbFetch!=='function')throw new Error('No se pudo conectar con el control de seguridad.');const r=await sbFetch('/rest/v1/rpc/security_lockdown_other_users',{method:'POST',body:{}});return r?.data||{}}
...
const labels={login_success:'Ingreso correcto',login_failure:'Intento fallido',login_blocked:'Ingreso bloqueado',login_rate_limited:'Límite de intentos',mfa_challenge_required:'Segundo factor requerido',mfa_failure:'Código 2FA incorrecto',mfa_enrollment_started:'Activación 2FA iniciada',mfa_enrolled:'2FA activado',mfa_unenrolled:'2FA desactivado',logout:'Cierre de sesión',session_revoked:'Sesiones cerradas por administrador',emergency_lockdown:'Cierre general de sesiones',account_disabled:'Usuario desactivado',account_reactivated:'Usuario reactivado',password_reset_by_admin:'Clave temporal restablecida',password_changed:'Contraseña personal actualizada',user_created:'Usuario creado'};
...
@media(max-width:440px){.cc-sec-kpis,.cc-sec-row,.cc-sec-event{grid-template-columns:1fr}.cc-sec-event>div:nth-child(2){grid-column:auto;grid-row:auto}.cc-sec-toolbar .actions{width:100%}.cc-sec-toolbar .actions .btn{flex:1}}
...
async function securityCenterModal(){
 ...
 async function load(){
  ...
  try{
   ...
   <section class="cc-sec-section"><h3>Sesiones recientes</h3><div class="cc-sec-list">${active.length?active.map(x=>{const u=userMap.get(x.user_id)||{},st=sessionState(x),self=x.user_id===session?.userId;return`<div class="cc-sec-row"><div><b>${E(u.full_name||x.email||'Usuario')}</b><small>${E(x.email||u.email||'')} · ${E(u.role||'')}${u.mfa_enabled?' · 2FA ACTIVO':''}</small></div><div><small>Dispositivo</small><b>${E(x.device_label||'No identificado')}</b></div><div><small>Última actividad</small><b>${E(ago(x.last_seen_at))}</b><small>${E(DT(x.started_at))}</small></div><div class="actions"><span class="status ${st[1]}">${self?'ESTA SESIÓN':st[0]}</span>${self?'':`<button class="btn danger" data-sec-revoke="${E(x.user_id)}" data-sec-name="${E(u.full_name||x.email||'usuario')}">Cerrar sesiones</button>`}</div></div>`}).join(''):'<div class="empty">No hay sesiones abiertas registradas.</div>'}</div></section>
   ...
   host.querySelector('[data-sec-lockdown]')?.addEventListener('click',async e=>{const b=e.currentTarget;if(!confirm('¿Cerrar todas las sesiones de los demás usuarios? Su sesión administrativa permanecerá abierta.'))return;if(!confirm('Confirme el cierre general. Los usuarios deberán ingresar nuevamente para recuperar el acceso.'))return;b.disabled=true;try{const x=await emergencyLockdown();toast(`Cierre general aplicado a ${Number(x?.affected_users)||0} usuario(s).`);await load()}catch(err){toast(err.message||'No se pudo aplicar el cierre general.')}finally{b.disabled=false}});
...
function bind(){
 ...
 const b=document.createElement('button');b.id='ccSecurityBtn';b.className='btn';b.type='button';b.textContent='Seguridad';b.title='Ingresos, 2FA, intentos fallidos y sesiones';b.onclick=securityCenterModal;
