/* ===== CENTRO DE SEGURIDAD Y ACCESOS V2 ===== */
(()=>{
'use strict';
...
async function securityCenterModal(){
 ...
 async function load(){
  ...
  try{
   ...
   <section class="cc-sec-section"><h3>Sesiones recientes</h3><div class="cc-sec-list">${active.length?active.map(x=>{const u=userMap.get(x.user_id)||{},st=sessionState(x),self=x.user_id===session?.userId;return`<div class="cc-sec-row"><div><b>${E(u.full_name||x.email||'Usuario')}</b><small>${E(x.email||u.email||'')} · ${E(u.role||'')}</small></div><div><small>Dispositivo</small><b>${E(x.device_label||'No identificado')}</b></div><div><small>Última actividad</small><b>${E(ago(x.last_seen_at))}</b><small>${E(DT(x.started_at))}</small></div><div class="actions"><span class="status ${st[1]}">${self?'ESTA SESIÓN':st[0]}</span>${self?'':`<button class="btn danger" data-sec-revoke="${E(x.user_id)}" data-sec-name="${E(u.full_name||x.email||'usuario')}">Cerrar sesiones</button>`}</div></div>`}).join(''):'<div class="empty">No hay sesiones abiertas registradas.</div>'}</div></section>
