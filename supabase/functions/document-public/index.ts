...
Deno.serve(async(req:Request)=>{
  try{
    ...
    if(!raw){
      ...
      const html=`<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(r.title||"Documento digital")}</title><style>body{margin:0;background:#07101c;color:#eef4fb;font-family:Inter,Arial,sans-serif}.wrap{max-width:1050px;margin:auto;padding:28px 18px}.card{background:#0e1927;border:1px solid #27415e;border-radius:18px;padding:20px;box-shadow:0 18px 60px rgba(0,0,0,.3)}.ey{font-size:11px;letter-spacing:.12em;color:#83b7ef;font-weight:800}.title{font-size:24px;margin:6px 0 4px}.meta{color:#9db0c4;font-size:13px}.btn{display:inline-block;margin-top:15px;background:#2563eb;color:white;text-decoration:none;padding:11px 16px;border-radius:11px;font-weight:800}.viewer{margin-top:16px;width:100%;height:75vh;border:0;border-radius:12px;background:white}@media(max-width:700px){.title{font-size:20px}.viewer{height:68vh}}</style></head><body><main class="wrap"><section class="card"><div class="ey">DOCUMENTO DIGITAL VERIFICADO</div><div class="title">${esc(r.title||"Informe")}</div><div class="meta">${esc(p?.code||"")} ${p?.name?`· ${esc(p.name)}`:""}<br>${esc(r.file_name||"")}</div><a class="btn" href="${open}" target="_blank" rel="noopener noreferrer">Abrir documento digital</a>${pdf?`<iframe class="viewer" src="${open}" title="Documento digital"></iframe>`:""}</section></main></body></html>`;
      return new Response(html,{headers:{...baseHeaders,"content-type":"text/html; charset=utf-8","x-frame-options":"DENY","content-security-policy":"default-src 'none'; style-src 'unsafe-inline'; frame-src 'self' https://*.supabase.co; img-src data:; connect-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'"}});
    }
    ...
    const mime=String(r.mime_type||"application/octet-stream"),isHtml=mime.toLowerCase().startsWith("text/html"),inline=/^(application\/pdf|text\/html|image\/)/i.test(mime);
    ...
    return new Response(bytes,{headers:{...baseHeaders,"content-type":mime,"content-disposition":`${inline?"inline":"attachment"}; filename*=UTF-8''${encodeURIComponent(r.file_name||"documento")}`,...htmlSecurity}});
