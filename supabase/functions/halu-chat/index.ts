...
function extractGeminiText(data:any){
  const out:string[]=[];
  const add=(x:any)=>{if(typeof x==="string"&&x.trim())out.push(x.trim())};
  add(data?.output_text);
  for(const item of data?.output||[]){add(item?.text);for(const part of item?.content||[])add(part?.text)}
  for(const step of data?.steps||[])for(const node of[step?.model_output,step?.output,step]){add(node?.text);add(node?.output_text);for(const part of node?.content||[])add(part?.text)}
  return [...new Set(out)].join("\n").trim();
}
function transcriptForProvider(history:Turn[],message:string,modeNote:string,context:string){
  const turns=history.map(turn=>(turn.role==="assistant"?"ZORDON: ":"Luis: ")+turn.text).join("\n");
  return zordonInstructions+"\n\n"+modeNote+(context?"\n\nContexto autorizado:\n"+context:"")+"\n\nHistorial reciente:\n"+(turns||"Sin historial reciente.")+"\n\nLuis: "+message+"\nZORDON:";
}
...
Deno.serve(async(req:Request)=>{
 ...
 try{
   ...
   return json(req,{reply,engine:"ZORDON",mode:casual?"casual":"normal",shared_memory:true,provider:result.provider,model:result.model});
