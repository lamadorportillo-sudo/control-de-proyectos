...
-- Periodo de preparación: los administradores existentes disponen de 7 días.
update public.profiles p
set mfa_required_after = coalesce(p.mfa_required_after, date_trunc('second', now() + interval '7 days')),
    updated_at = now()
where exists (
  select 1
...
as $$
  select exists (
    ...
    where p.user_id = auth.uid()
      ...
      -- Si el usuario decidió activar MFA, toda sesión debe ser AAL2.
      and (
        not exists (
          select 1
      ...
      -- Para administradores, después de la fecha de preparación ya es obligatorio tener MFA.
      and (
        not exists (
          select 1
          from public.workspace_members wm
