...
assert.match(contrast,/CONTRASTE DOCUMENTAL V3/,'debe existir la corrección contextual V3 del expediente documental');
...
assert.match(contrast,/\.cc-payment-docs\{/,'la tarjeta contractual debe declarar una base sólida además del degradado');
assert.match(contrast,/\.cc-payment-docs \.cc-payment-docs-kicker/,'el encabezado EXPEDIENTE CONTRACTUAL debe tener contraste propio');
assert.match(contrast,/\.cc-payment-docs \.cc-payment-docs-head p/,'la explicación del bloque contractual debe tener contraste propio');
...
assert.match(contrast,/\.cc2-payment-head\{/,'cada cabecera de pago debe declarar una superficie clara explícita');
assert.match(contrast,/\.cc2-payment-head small\{/,'las etiquetas y fechas de cada pago deben conservar contraste AA sin depender de atributos inexistentes');
assert.doesNotMatch(contrast,/\.cc2-payment-head small\[data-cc-readable=/,'la corrección no debe depender de un atributo que el DOM real no renderiza');
...
assert.equal(supplementalModules[contrastAt][1],'20260905-contractdoccontrast3');

console.log('contract-ui-invariants: monto vigente calculado y expediente documental V3 con contraste AA contextual');
