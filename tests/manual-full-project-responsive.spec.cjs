...
test('simulación manual integral de un proyecto municipal inventado',async({page},ti)=>{
 ...
 try{
  ...
  for(let i=0;i<P.offers.length;i++){
   const [bid,amount,corr]=P.offers[i];await click(page,'#addOffer',m);if(i===0)await modal(page,'Agregar oferta',m);await fill(page,'#ofBidder',bid,m);await fill(page,'#ofAmount',amount,m);await fill(page,'#ofCorrected',corr,m);await sel(page,'#ofTech','Cumple',m);await sel(page,'#ofEligible','Sí',m);await fill(page,'#ofNotes',i===2?'Corrección aritmética registrada en el monto evaluado.':'Oferta técnica y documentalmente admisible.',m);await click(page,'#offerForm button.btn.primary',m);
  }
  ...
  await fill(page,'#vDate','2026-09-18',m);await sel(page,'#vType','Supervisión',m);await sel(page,'#vStatus','Con observaciones',m);await fill(page,'#vObjective','Verificar tubería sanitaria, pozos, relleno compactado y preparación para pavimento.',m);await fill(page,'#vPhysical','18',m);await fill(page,'#vPersonnel','14',m);await fill(page,'#vWeather','Soleado, terreno seco',m);await fill(page,'#vContractorRep','Ing. Andrea López',m);await fill(page,'#vSupervisor','Ing. Carlos Mendoza',m);await fill(page,'#vNext','2026-09-25',m);await fill(page,'#vActivities','Excavación, instalación de tubería PVC, pozos, relleno por capas y subrasante.',m);await fill(page,'#vGeneralObs','Un tramo presenta mayor humedad; verificar compactación antes de continuar pavimento.',m);await fill(page,'#vInstructions','Realizar control de compactación y comprobar cotas de invert.',m);await fill(page,'#vCommitments','Contratista presentará controles y corregirá tramos que no cumplan.',m);await click(page,'#visitForm button.btn.primary',m);await expect(page.locator('#tabBody')).toContainText('Ing. Andrea López');await shot(page,ti,'09-visita-de-obra');

  // ORDEN DE CAMBIO
  await tab(page,'changes',m);await click(page,'#newCh',m);await modal(page,'Orden de cambio',m);await fill(page,'#chNumber','OC-01',m);await fill(page,'#chDate','2026-09-22',m);await fill(page,'#chAmount','85000',m);await fill(page,'#chDays','10',m);await sel(page,'#chStatus','Aprobado',m);await fill(page,'#chJust','Tramo adicional de colector y dos pozos por condición encontrada en campo, con revisión técnica municipal.',m);await click(page,'#chForm button.btn.primary',m);await expect(page.locator('#tabBody')).toContainText('OC-01');await shot(page,ti,'10-orden-de-cambio');
