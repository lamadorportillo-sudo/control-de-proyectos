/* ===== ZORDON · BUSCADOR INTELIGENTE DE PROYECTOS V2 ===== */
(()=>{
'use strict';
if(typeof window==='undefined'||typeof document==='undefined')return;
if(window.__CC_ZORDON_PROJECT_SEARCH_V2__)return;
window.__CC_ZORDON_PROJECT_SEARCH_V2__=true;
window.__CC_ZORDON_PROJECT_SEARCH_V1__=true;

const STYLE_ID='cc-zordon-project-search-v2-style';
...
const ALIASES={
  parque:['parque','plaza','central'],iglesia:['iglesia','catolica','templo','ermita','hermita'],
  pavimento:['pavimento','pavimentacion','calle','concreto','huella','huellas'],colegio:['colegio','escuela','instituto','kinder','jardin'],
  electrico:['electrico','electrica','electricidad','linea','primaria','secundaria','transformador','transformadores'],agua:['agua','tanque','pozo','sistema','hidrico'],
  tanque:['tanque','agua','almacenamiento'],estadio:['estadio','campo','cancha'],comunal:['comunal','social','centro'],
  ejecucion:['ejecucion','ejecutando','activo','activa','vigente'],finalizado:['finalizado','finalizada','terminado','terminada','cerrado','cerrada'],
  atencion:['atencion','seguimiento','alerta','riesgo','atraso','atrasado','atrasada'],contratista:['contratista','constructor','constructora','ingeniero','ingeniera']
};
const norm=v=>String(v??'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9.%]+/g,' ').replace(/\s+/g,' ').trim();
const esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
const words=v=>norm(v).split(' ').filter(Boolean);
function significant(q){const out=[];words(q).forEach(t=>{if(STOP.has(t))return;out.push(t);for(const [key,arr] of Object.entries(ALIASES))if(t===key||arr.includes(t))arr.forEach(x=>out.push(x))});return[...new Set(out)]}
...
function projectDataForCard(card){
  const code=norm(card.querySelector('.project-v3-code')?.textContent||'');const store=getDB();if(!store||!code)return null;
...
function corpus(card){
  ...
  if(p){extra+=flatObject(p);try{const store=getDB();const contracts=(store?.contracts||[]).filter(c=>c.projectId===p.id);extra+=flatObject(contracts);const ids=new Set(contracts.map(c=>c.contractorId||c.contractor_id).filter(Boolean));extra+=flatObject((store?.contractors||[]).filter(c=>ids.has(c.id)))}catch{}}
  return norm(`${card.textContent||''} ${extra}`);
}
function closeEnough(token,text){
  if(!token)return true;if(text.includes(token))return true;if(token.length<5)return false;
  const candidates=text.split(' ').filter(w=>Math.abs(w.length-token.length)<=1&&w.length>=4);
  return candidates.some(w=>{if(w[0]!==token[0])return false;let i=0,j=0,d=0;while(i<token.length&&j<w.length){if(token[i]===w[j]){i++;j++;continue}d++;if(d>1)return false;if(token.length>w.length)i++;else if(w.length>token.length)j++;else{i++;j++}}d+=token.length-i+w.length-j;return d<=1});
}
function scoreCard(card,q){
  const text=corpus(card),tokens=significant(q);if(!tokens.length)return 0;const original=norm(q);let score=0,matchedCore=0;if(original&&text.includes(original))score+=100;
  const rawCore=words(q).filter(t=>!STOP.has(t));rawCore.forEach(t=>{if(closeEnough(t,text)){matchedCore++;score+=18+(t.length>6?4:0)}});tokens.forEach(t=>{if(closeEnough(t,text))score+=3});
  if(rawCore.length&&matchedCore===rawCore.length)score+=35;if(rawCore.length>1&&matchedCore<Math.ceil(rawCore.length*.6))return 0;if(rawCore.length===1&&matchedCore===0)return 0;return score;
}
function installCss(){
...
function apply(board,query){
  const grid=board.querySelector('.project-grid-v3');if(!grid)return;const cards=[...grid.querySelectorAll(':scope > .project-v3')],q=String(query||'').trim(),count=board.querySelector('[data-zordon-count]');
  if(!q){cards.forEach(c=>c.style.display='none');showState(grid,'Zordon está listo','Escribe el nombre, código, contratista, ubicación, estado o una frase como “proyectos en ejecución”.');if(count)count.textContent='Esperando búsqueda';return}
  ...
  const ranked=cards.map(c=>({c,score:scoreCard(c,q)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score);cards.forEach(c=>c.style.display='none');ranked.forEach(({c})=>c.style.display='');
  if(ranked.length){hideState(grid);if(count)count.textContent=`${ranked.length} resultado${ranked.length===1?'':'s'}`}else{showState(grid,'No encontré coincidencias',`No encontré un proyecto para “${q}”. Prueba con código, nombre, contratista, comunidad, tipo de obra o estado.`);if(count)count.textContent='0 resultados'}
}
function mount(board){
  installCss();if(!board||!board.querySelector('.project-grid-v3'))return false;
  let box=board.querySelector(WRAP);
  if(!box){
    /* El buscador Zordon adopta el ID oficial. Así la UI, navegación y pruebas usan un solo buscador. */
    ...
    box.innerHTML=`<div class="zordon-project-search-head"><div><small>ZORDON · BÚSQUEDA INTELIGENTE</small><b>¿Qué proyecto quieres ver?</b><span>Escribe como hablas: nombre, código, contratista, comunidad, tipo de obra o estado.</span></div><span class="zordon-project-count" data-zordon-count>Esperando búsqueda</span></div><div class="zordon-project-search-row"><input id="projectSearch" class="zordon-project-search-input search" data-zordon-input data-cc-search-clean="1" type="search" autocomplete="off" spellcheck="false" placeholder="Ej.: parque central · contratista Harold · pavimento colegio · proyectos en ejecución"><button type="button" class="btn zordon-project-clear" data-zordon-clear>Limpiar</button></div><div class="zordon-project-hints"><button type="button" data-zordon-q="parque central">Parque</button><button type="button" data-zordon-q="pavimento">Pavimento</button><button type="button" data-zordon-q="iglesia">Iglesia</button><button type="button" data-zordon-q="en ejecución">En ejecución</button><button type="button" data-zordon-q="finalizados">Finalizados</button></div>`;
    ...
    input.addEventListener('input',()=>apply(board,input.value));input.addEventListener('keydown',e=>{if(e.key==='Escape'){input.value='';apply(board,'');input.blur()}});
    box.querySelector('[data-zordon-clear]').addEventListener('click',()=>{input.value='';apply(board,'');input.focus()});box.querySelectorAll('[data-zordon-q]').forEach(btn=>btn.addEventListener('click',()=>{input.value=btn.dataset.zordonQ||'';apply(board,input.value);input.focus()}));
  }
...
let queued=false;const observer=typeof MutationObserver==='function'?new MutationObserver(()=>{if(queued)return;queued=true;const go=()=>{queued=false;run()};if(typeof requestAnimationFrame==='function')requestAnimationFrame(go);else go()}):null;
observer?.observe(document.documentElement,{childList:true,subtree:true});setTimeout(run,0);setTimeout(run,350);setTimeout(run,1100);
